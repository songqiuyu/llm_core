...
| 隐藏维度 E | 4096 |
| Q 头数 H | 32 |
| KV 头数 KVH | 32（标准 MHA，非 GQA） |
| Head Dim HD | 128 |
| FFN 中间维度 FD | 11008 |
| 词表大小 V | 32000 |
| RoPE theta | 10000.0（GGUF 未存，取默认值） |
| RMSNorm eps | 1e-5（GGUF 存为 0，fallback 取默认值） |
...
scale/min 解包（`get_scale_min_k4`，匹配 GGML 参考实现）：
- j < 4：`d = scales[j] & 0x3F`，`m = scales[j+4] & 0x3F`
- j ≥ 4：
  ```
  d = (scales[j+4] & 0x0F) | ((scales[j-4] >> 6) << 4)   // 取 j+4 的低 nibble
  m = (scales[j+4] >>   4) | ((scales[j  ] >> 6) << 4)   // 取 j+4 的高 nibble
  ```
  j 和 j-4 的高 2 位（bit 7:6）分别贡献到 d/m 的高 2 位。
...
### 问题 5：`q4k_get_scale_min` j≥4 分支 nibble 提取错误 ⭐ 根本原因

**现象**：TinyLlama-1.1B 生成结果与 Python 参考一致，但整体输出语义不合理。LLaMA-2-7B 输出完全乱码（重复 "riiriiri" 等），且所有后续 token 的 logits 完全相同。

**调试过程**：

在 LLaMA-2-7B forward pass 中逐层追踪残差向量的 RMS：

```
pos=1 各层 RMS:
l=0:  0.01   l=1:  0.05   l=2:  0.08   l=3:  0.25
l=4:  105.5  ← 单层爆炸 420x！
l=5:  106.3  ...
l=31: 7995.0  ← 已完全发散
```

第 4 层入口 RMS 从 0.25 跳变到 105（420 倍），锁定到第 3 层 FFN。
进一步打印发现 `ffn[0] = -85.4`，但 Python 参考（用真实 x 向量）计算出的 `ffn[0] ≈ 0.7`，**相差 120 倍**。

**根本原因**：`q4k_get_scale_min` 对 j≥4 的分支（处理 chunk 2、3，即每个 256 元素块的后半段）使用了错误的 nibble 位置：

```cpp
// 错误（旧代码）：
d = (sc[j+4] >> 4)    | ((sc[j-4] & 0xC0) >> 2);  // 取了高 nibble，应取低 nibble
m = (sc[j+0] >> 4)    | ((sc[j-4] & 0xC0) >> 2);  // 取了 sc[j]，应取 sc[j+4]

// 正确（GGML 参考）：
d = (sc[j+4] & 0x0F)  | ((sc[j-4] >> 6) << 4);    // 取 j+4 低 nibble
m = (sc[j+4] >>    4) | ((sc[j-0] >> 6) << 4);    // 取 j+4 高 nibble
```

**影响范围**：每个 256 元素 Q4K 块中 chunk=2,3 对应的 128 个元素（占 50%）的 scale 和 min 值完全错误，导致反量化权重普遍偏大 10-100 倍。

**为何 TinyLlama 的 Python 参考"验证通过"**：验证脚本 `/tmp/debug7b.py` 复现了相同的错误公式，两者"一致"但都是错的。这掩盖了该 bug 近乎整个调试周期。

**修复**（一行）：
```cpp
// llama_model.cpp, q4k_get_scale_min, j>=4 branch
d = (sc[j+4] & 0x0F) | ((sc[j-4] >> 6) << 4);
m = (sc[j+4] >>   4) | ((sc[j-0] >> 6) << 4);
```

**修复后结果**：
- 残差 RMS 最终层从 7995 降至 **~2.0**（正常范围）
- TinyLlama: `"the city of Paris, which is the capital of France."` ✓
- LLaMA-2-7B: `"a city of contrasts. The city of Paris is a..."` ✓

---

### 问题 6：`sample_top_k` temperature=0 除零崩溃

**现象**：以 `-t 0.0`（贪心采样）调用时，`temperature` 为 0，`logits[i] /= temperature` 产生 Inf/NaN，softmax 结果不确定。

**根本原因**：`sample_top_k` 直接用 temperature 做除法，未处理 temperature=0 特殊情况。

**修复**：在函数入口添加贪心 argmax 短路：
```cpp
...
| 组件 | 状态 | 说明 |
|------|------|------|
| Q4_K_M 反量化内核 | ✅ | `q4k_dot_row` + `q4k_row_to_f32`，nibble 公式已对齐 GGML |
...
| GQA / MHA 注意力 | ✅ | TinyLlama GQA(KVH=4)，LLaMA-2-7B MHA(KVH=32) |
...
| temperature=0 贪心采样 | ✅ | argmax 短路，避免 temperature 除零 |
| 数值正确性 | ✅ | 两个模型均产生语义正确的文本输出 |

### 生成质量验证（问题 5 修复后）

prompt: `"The capital of France is"` → greedy decode, `-t 0.0`

| 模型 | 生成文本 | 评价 |
|------|----------|------|
| TinyLlama-1.1B Q4_K_M | `the city of Paris, which is the capital of France.` | ✅ 正确且语义完整 |
| LLaMA-2-7B Q4_K_M | `a city of contrasts. The city of Paris is a...` | ✅ Paris 出现，语义通顺 |

### 性能（当前基线，纯标量，无 SIMD 优化）

| 模型 | TTFT | 吞吐 |
|------|------|------|
| TinyLlama-1.1B Q4_K_M | ~4900 ms | ~0.96 tok/s |
| LLaMA-2-7B Q4_K_M | ~24700 ms | ~0.18 tok/s |

**瓶颈分析**：Q4K/Q6K 完全标量实现，没有 SIMD 加速。每个 GEMV 行的反量化+点积为纯 C++ 循环，远未达到内存带宽上限（DDR4-3200 理论 51.2 GB/s）。LLaMA-2-7B 的权重矩阵是 TinyLlama 的 ~8 倍，吞吐相应下降。
...
### 8.3 LLaMA-2 7B 验证 ✅ 已完成

`models/llama-2-7B/llama-2-7b.Q4_K_M.gguf` 是基座模型，无 chat template 要求。在修复问题 5 后：

```bash
build/tools/cli/axonforge-cli -m models/llama-2-7B/llama-2-7b.Q4_K_M.gguf \
    -p "The capital of France is" -n 12 -t 0.0
# 输出: a city of contrasts. The city of Paris is a...
```

输出语义正确，"Paris" 出现。TTFT ~24.7s，吞吐 ~0.18 tok/s（标量瓶颈，待 AVX2 优化）。

### 8.4 性能目标（路线图）

| 阶段 | 优化内容 | 预期吞吐（TinyLlama-1.1B） |
|------|----------|--------------------------|
| 当前（已完成） | 标量 Q4K/Q6K | ~0.96 tok/s |
| Phase 2a | AVX2 Q4K GEMV | ~5-10 tok/s |
| Phase 2b | AVX2 Q6K + attention SIMD | ~10 tok/s |
| Phase 2c | AVX-VNNI INT8 路径 | ~15 tok/s |
...
| `src/models/llama_model.cpp` | 全部量化实现：WT/gemv_wt/emb_lookup/q4k/q6k 内核，RoPE/GQA/SwiGLU 前向，BOS 插入，SP ▁ 前缀修复；**修复 `q4k_get_scale_min` j≥4 nibble 错误**；**修复 `sample_top_k` temperature=0 除零** |
