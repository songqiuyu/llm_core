...
范围限定为 dense Qwen2/Qwen2.5 和 DeepSeek-R1-Distill-Qwen。暂不支持 Qwen2-MoE、Qwen3、Qwen3-MoE、DeepSeek-V2/V3/R1 原版 MoE/MLA。
...
`Engine::encode/decode` 和 `Session::generate` 在 `general.architecture == "qwen2"` 时分发到 Qwen2 路径。CLI 也显式识别 `qwen2`，保持 streaming callback 与统计输出一致。
