...
std::vector<int32_t> qwen2_generate(
    const Engine& engine,
...
static std::vector<std::string> qwen2_pretokenize(std::string_view text) {
    ...
    for (size_t i = 0; i < cps.size(); ) {
        ...
        if (cp == '\'' && i + 1 < cps.size()) {
            const uint32_t n1 = static_cast<uint32_t>(std::tolower(static_cast<unsigned char>(cp_at(i + 1))));
            if (n1 == 's' || n1 == 't' || n1 == 'm' || n1 == 'd') {
                out.push_back(cps[i] + cps[i + 1]);
                i += 2;
                continue;
            }
            if (i + 2 < cps.size()) {
                const uint32_t n2 = static_cast<uint32_t>(std::tolower(static_cast<unsigned char>(cp_at(i + 2))));
                if ((n1 == 'r' && n2 == 'e') || (n1 == 'v' && n2 == 'e') || (n1 == 'l' && n2 == 'l')) {
                    out.push_back(cps[i] + cps[i + 1] + cps[i + 2]);
                    i += 3;
                    continue;
                }
            }
        ...
        std::string seg;
        if (!is_space(cp) && !is_digit(cp) && (is_ascii_letter(cp) || (i + 1 < cps.size() && is_ascii_letter(cp_at(i + 1))))) {
            if (!is_ascii_letter(cp)) seg += cps[i++];
            while (i < cps.size() && is_ascii_letter(cp_at(i))) seg += cps[i++];
            if (!seg.empty()) { out.push_back(seg); continue; }
        }
        if (is_digit(cp)) {
            out.push_back(cps[i++]);
            continue;
        }
        if ((cp == ' ' && i + 1 < cps.size() && !is_space(cp_at(i + 1)) && !is_ascii_letter(cp_at(i + 1)) && !is_digit(cp_at(i + 1))) ||
            (!is_space(cp) && !is_ascii_letter(cp) && !is_digit(cp))) {
            if (cp == ' ') seg += cps[i++];
            while (i < cps.size() && !is_space(cp_at(i)) && !is_ascii_letter(cp_at(i)) && !is_digit(cp_at(i))) seg += cps[i++];
...
static bool token_is_special(const std::string& tok, int32_t type) {
    return type == 3 || type == 4 || (tok.size() >= 4 && tok[0] == '<' && tok[1] == '|');
}

} // namespace

std::vector<int32_t> qwen2_encode(const Engine& engine,
                                  std::string_view text,
                                  bool raw) {
    const auto& vocab = engine.vocabulary();
    const auto& merges = engine.tokenizer_merges();
    if (vocab.empty())
        throw std::runtime_error("qwen2_encode: vocabulary is empty");
    if (merges.empty())
        throw std::runtime_error("qwen2_encode: tokenizer.ggml.merges is empty");
    ...
    auto bpe_segment = [&](std::string_view seg, std::vector<int32_t>& ids) {
        ...
        for (const std::string& piece : qwen2_pretokenize(seg)) {
...
std::string qwen2_decode(const Engine& engine,
